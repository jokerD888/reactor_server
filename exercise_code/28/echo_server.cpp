...
void EchoServer::OnMessage(std::shared_ptr<Connection> conn, std::string& message) {
    // printf("%s  connection %d message: %s\n", TimeStamp::Now().ToString().c_str(), conn->fd(), message.data());

...
void Connection::SendInLoop(std::string data) {
    output_buffer_.AppendWithSep(data.data(), data.size());
    // 注册写事件
    client_channel_->EnableWriting();
}
void Connection::Send(const char* data, size_t size) {
    ...
    if (loop_->IsInLoopThread()) {  // 当前线程为事件循环线程（IO线程）
        ...
        // printf("send data in work thread\n");
...
void Connection::OnMessage() {
    ...
    while (true) {  // 由于使用非阻塞IO，需要循环读取，直到没有数据可读
        ...
        if (nread > 0) {
            ...
            // 没有数据可读，则从input_buffer_提取完整的消息
            // EAGAIN 和 EWOULDBLOCK 本质相同，表示非阻塞操作无法立即完成。
            // printf("recv(eventfd=%d):%s\n", fd(), input_buffer_.data());
            std::string message;
            while (true) {
                if (!input_buffer_.PickMessage(message)) break;
                // printf("message (eventfd=%d):%s\n", fd(), message.c_str());
                last_time_ = TimeStamp::Now();
                // printf("last_time_ = %s\n", last_time_.ToString().c_str());
...
bool Connection::IsTimeout(time_t now, int val) { return now - last_time_.ToInt() > val; }